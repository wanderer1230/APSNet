...
import data_manager
from cam_functions import get_cam, visual_batch
...
parser.add_argument('--test_path', type=str, default="F:/BaiduNetdiskDownload/rank1_0.91161615_checkpoint_ep499.pth")
...
def test(model, queryloader, galleryloader, pool, use_gpu, dataset, ranks=[1, 5, 10, 20]):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, pids, camids, img_path) in enumerate(tqdm(queryloader)):
            ...
            features,cls ,feature_3= model(imgs, pids, camids)
            fig = plt.figure()
            ax = fig.add_subplot(221)
            img1 = imgs[0][0]
            raw_image = img1.cpu().numpy().transpose(1, 2, 0)
            ax.imshow(raw_image)
            #fig.savefig("ttt.jpg")
            plt.show()
            weight = model.module.classifier[0].weight #625 x 1024
            a = cls[3].reshape(1024,16,8)
            cam = get_cam(a,1, feature_3[3])
            #weight = weight.reshape(1024,625)
            visual_batch(cam , img1,"1"," ","hh")
        ...
        for batch_idx, (imgs, pids, camids, img_path) in enumerate(tqdm(galleryloader)):
            ...
            features, pids,features_4 = model(imgs, pids, camids)
            features = features.data.cpu()
            torch.cuda.empty_cache()
            features = features.view(-1, features.size(1))
